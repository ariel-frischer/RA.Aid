"""Utilities for handling token limits with Anthropic models with context condensation support."""
...
async def condensed_base_state_modifier(
    ...
    condensed_remaining = await condenser.condense_messages(
        remaining_messages,
        token_counter=token_counter,
        max_tokens=max_input_tokens - token_counter([first_message]),
        num_messages_to_keep=2,
        preserve_system_messages=True,
    )
...
def get_condenser_config(
    ...
    """Get the condenser configuration from the config dictionary.
    
    Args:
        config: Configuration dictionary
        
    Returns:
        Tuple[bool, str, Optional[str], str]: (enable_condensation, condenser_model, condenser_provider, condensation_quality)
    """
    try:
        # Try to use repository config for production use
        try:
        ...
        if repository_available:
            enable_condensation = repo.get("enable_condensation", True)
            condenser_model = repo.get("condenser_model", None)
            condenser_provider = repo.get("condenser_provider", None)
            condensation_quality = repo.get("condensation_quality", "balanced")
        else:
            enable_condensation = config.get("enable_condensation", True)
            condenser_model = config.get("condenser_model", None)
            condenser_provider = config.get("condenser_provider", None)
            condensation_quality = config.get("condensation_quality", "balanced")
