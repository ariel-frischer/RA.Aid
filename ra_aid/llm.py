...
def create_deepseek_client(
    ...
    if model_name.lower() == "deepseek-reasoner":
        print(f'using custom ChatDeepseekReasoner{" for expert model" if is_expert else ""}')
...
def create_openrouter_client(
    ...
    if model_name.startswith("deepseek/") and "deepseek-r1" in model_name.lower():
        print(f'using custom ChatDeepseekReasoner via OpenRouter{" for expert model" if is_expert else ""}')
        return ChatDeepseekReasoner(
            api_key=api_key,
