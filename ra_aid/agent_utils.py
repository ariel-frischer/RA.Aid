...
def is_gemini_flash(config: Dict[str, Any]) -> bool:
    """Check if the model name is Google's Gemini 2.0 Flash.

    Args:
        config: Configuration dictionary containing model information

    Returns:
        bool: True if this is Google's Gemini 2.0 Flash model
    """
    model_name = config.get("model", "")
    return "gemini-2.0-flash-001" in model_name
...
def create_agent(
    ...
    *,
    ...
    try:
        ...
        # Use REACT agent for Anthropic Claude models or Gemini Flash, otherwise use CIAYN
        if is_anthropic_claude(config) or is_gemini_flash(config):
