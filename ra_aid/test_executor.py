...
class TestExecutor:
    ...
    def display_test_failure(self, attempts: int) -> None:
        ...
        console.print(
            Panel(
                Markdown(f"Test failed. Attempt number {attempts} of {self.max_test_retries}. Retrying and informing of failure output"),
    ...
    def execute(
        ...
        if not self.auto_test:
            print()
            response = ask_human("Would you like to run the test command? (y=yes, n=no, a=enable auto-test)")
